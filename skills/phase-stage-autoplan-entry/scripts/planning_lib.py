...
def runtime_phase_handoff_target(task_context: dict[str, Any]) -> str | None:
    project_root = Path(task_context["projectRoot"]).resolve()
    run_root = project_root / ".codex" / "phase-stage-autorun" / task_context["runId"]
    package_path = run_root / "run-package.aclx"
    if not package_path.exists():
        return None
    entries = read_aclx(package_path)
    if entries.get("stage.blocker") != "missing_plan" or entries.get("stage.next") != "expand_phase_plan":
        return None
    phase_id = entries.get("verify.latest")
