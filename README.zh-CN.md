...
- 你要处理的是长工程、复杂工程、跨多个阶段的任务，而不是一次性的小修小补
- 你希望执行过程自动持续推进，不需要人工一轮一轮催促、确认和监督
- 你希望每一个 stage 构建结束后都经过 `$generator-critic-verification-loop` 审计，再进入下一个 stage，以保证自动推进时的准确性
- 你希望用 ACL-X 控制策略显著降低长循环、阶段推进和恢复执行时的 token 与时间消耗
